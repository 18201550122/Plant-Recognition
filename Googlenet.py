...
'''                       可调代码点    加载数据            ******************************************************************************               '''
train_data = torchvision.datasets.ImageFolder(root=dataPath+"\\train", transform=data_transform["train"])  # 训练集

traindata = DataLoader(dataset=train_data, batch_size=32, shuffle=True, num_workers=0)  # 将训练数据以每次32张图片的形式抽出进行训练

test_data = torchvision.datasets.ImageFolder(root=dataPath+"\\val", transform=data_transform["val"])  # 将训练数据以每次32张图片的形式抽出进行测试
...
googlenet = GoogLeNet(num_classes=46, aux_logits=True, init_weights=True)  # 启动模型，这里的 num_classes 就改成自己的数据集的种类即可，几种就改成几
'''                       可调代码点         自己的数据集是几种，这个 num_classes 就设置为几       ******************************************************************************               '''
'''                       可调代码点                ******************************************************************************               '''
...
# print(nowt)
plt.savefig('googlenet'+nowt+str(r1)+'.png')
'''                       可调代码点       结果保存         ******************************************************************************               '''

plt.show()

torch.save(googlenet.state_dict(), "googlenet"+nowt+str(r1)+".pth")  # 保存模型
'''                       可调代码点       模型保存         ******************************************************************************               '''
print("模型已保存")
